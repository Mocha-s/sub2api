...
func (s *VideoTaskService) Create(ctx context.Context, params VideoTaskCreateParams) (*VideoTaskCreateResult, error) {
	...
	for {
		...
		if endpoint == VideoTaskEndpointVideoGenerations {
			if err := validateVideoCompatCreateBody(forwardBody, upstreamModel); err != nil {
				if selection.ReleaseFunc != nil {
					selection.ReleaseFunc()
				}
				return nil, err
...
func (s *VideoTaskService) Estimate(ctx context.Context, params VideoTaskEstimateParams) (*VideoTaskEstimateResult, error) {
	...
	if quote != nil {
		result, err = applyVideoTaskEstimateQuote(result, *quote)
		if err != nil {
			return nil, err
		}
	}
	if publicModel, ok := RequestedPublicModelFromContext(ctx); ok {
		return applyVideoTaskEstimatePublicModel(result, publicModel)
	}
	return result, nil
}

func applyVideoTaskEstimatePublicModel(result *VideoTaskEstimateResult, publicModel string) (*VideoTaskEstimateResult, error) {
	if result == nil {
		return nil, nil
	}
	...
	if result.Metadata != nil {
		if _, ok := result.Metadata["model"]; ok {
			result.Metadata["model"] = publicModel
		}
	}
	return result, nil
}
