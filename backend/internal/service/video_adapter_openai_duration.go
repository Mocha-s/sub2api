...
func openAIDurationCreateBody(body []byte, upstreamModel string) ([]byte, error) {
	...
	for _, key := range []string{"model", "prompt", "aspect_ratio", "resolution", "generate_audio", "content", "images", "videos", "audios"} {
...
func openAIDurationResultURL(raw []byte) string {
	...
	if resultURL := openAIDurationURLFromMap(payload, "video_url", "videoUrl", "url"); resultURL != "" {
		return resultURL
	}
	...
	if metadata, ok := payload["metadata"].(map[string]any); ok {
		if resultURL := openAIDurationURLFromMap(metadata, "url"); resultURL != "" {
			return resultURL
		}
	}
	return ""
