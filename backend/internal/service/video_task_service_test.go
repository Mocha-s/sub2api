...
func TestVideoTaskServiceEstimateCompositePreservesPublicAliasAndUsesMappedVideoPricing(t *testing.T) {
	ctx := WithCompositeRouteDecision(context.Background(), CompositeRouteDecision{
		Matched:        true,
		Source:         CompositeRouteSourceExplicit,
		PublicModel:    "video-alias",
	...
	selector := &fakeVideoTaskSelector{selection: &AccountSelectionResult{Account: &Account{
		ID:       99,
		Platform: PlatformOpenAI,
		Credentials: map[string]any{
			"model_mapping": map[string]any{"sora-upstream": "provider-sora"},
	...
	provider := &accountVideoTaskProvider{registry: videoAdapterRegistry{VideoAdapterJimengOpenAIVideos: &jimengOpenAIVideosAdapter{provider: &fakeVideoTaskProvider{}}}}
	price, defaultSeconds := 0.08, 5
	...
	result, err := svc.Estimate(ctx, VideoTaskEstimateParams{
		APIKey:      videoTaskCompositeTestAPIKey(true),
		User:        &User{ID: 7},
		Body:        []byte(`{"model":"sora-upstream","prompt":"waves","duration":5}`),
		Endpoint:    VideoTaskEndpointVideoGenerations,
		ContentType: "application/json",
	})
	...
	var response map[string]any
	require.NoError(t, json.Unmarshal(result.ResponseBody, &response))
	require.Equal(t, "video-alias", response["model"])
	require.Equal(t, "provider-sora", response["upstream_model"])
	require.Equal(t, "provider-sora", response["billing_model"])
	require.Equal(t, "sora-upstream", selector.lastModel)
	require.Equal(t, "video-alias", pricing.lastInput.RequestedModel)
	require.Equal(t, "provider-sora", pricing.lastInput.UpstreamModel)
}

func TestVideoTaskServiceEstimateVideoRejectsMappedCompatBeforeEstimator(t *testing.T) {
	selector := &fakeVideoTaskSelector{selection: &AccountSelectionResult{Account: &Account{
		ID:       99,
		Platform: PlatformOpenAI,
		Credentials: map[string]any{
			"model_mapping": map[string]any{"public-video": "kling-o3"},
		},
	}}}
	provider := &fakeVideoTaskEstimator{fakeVideoTaskProvider: &fakeVideoTaskProvider{}}
	svc := newVideoTaskServiceForTest(newFakeVideoTaskRepository(nil), nil, selector, provider, nil)
	body := []byte(`{"model":"public-video","prompt":"city","duration":5,"audio_reference":"https://cdn.example/audio.wav"}`)
	...
	require.Zero(t, provider.estimateCalls)
	require.Equal(t, "public-video", selector.lastModel)
}
