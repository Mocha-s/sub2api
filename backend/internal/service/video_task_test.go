...
func TestParseOpenAIVideoCreateRequest_DocumentedFields(t *testing.T) {
	originalBody := []byte(`{"model":"  sora-2  ","prompt":"  slow dolly over city lights  ","seconds":8,"aspect_ratio":" 16:9 ","images":[{"url":"https://example.test/image.png"}],"videos":[{"url":"https://example.test/input.mp4"}],"audios":[{"url":"https://example.test/input.wav"}],"provider_extra":"kept out of typed fields"}`)
...
func TestParseOpenAIVideoCreateRequest_RequiresModelAndPrompt(t *testing.T) {
	tests := []struct {
		...
		{name: "missing prompt", body: []byte(`{"model":"sora-2"}`)},
		{name: "blank prompt", body: []byte(`{"model":"sora-2","prompt":" \n "}`)},
	}
