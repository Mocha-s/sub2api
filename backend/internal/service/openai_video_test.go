...
func TestOpenAICompatibleVideoProviderCreateSendsRequestAndParsesResponse(t *testing.T) {
	body := []byte(`{"model":"sora-2","prompt":"slow dolly"}`)
	...
	result, err := provider.Create(context.Background(), account, body, contentType, "sora-2")
	if err != nil {
...
func TestOpenAICompatibleVideoProviderForGatewayUsesGatewayTokenProxyAndHTTPUpstream(t *testing.T) {
	...
	result, err := provider.Create(context.Background(), account, []byte(`{"model":"sora-2","prompt":"slow dolly"}`), "application/json", "sora-2")
...
func TestOpenAICompatibleVideoProviderCreateRewritesModelToUpstreamModel(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotBody, err := io.ReadAll(r.Body)
		if err != nil {
			t.Errorf("ReadAll request body: %v", err)
		...
		if got := payload["model"]; got != "upstream-video" {
			t.Errorf("model = %#v, want upstream-video", got)
		}
		if got := payload["prompt"]; got != "slow dolly" {
			t.Errorf("prompt = %#v, want preserved prompt", got)
		}

		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"id":"video_model_rewrite","status":"queued"}`)
	}))
	defer server.Close()

	provider := NewOpenAICompatibleVideoProvider(server.Client())
	account := &Account{Credentials: map[string]any{"base_url": server.URL, "api_key": "sk-video"}}
	body := []byte(`{"model":"client-video","prompt":"slow dolly"}`)
