...
func (p *openAICompatibleVideoProvider) Create(ctx context.Context, account *Account, body []byte, contentType string, upstreamModel string) (*VideoProviderCreateResult, error) {
	...
	forwardBody, err := rewriteOpenAIVideoCreateModel(body, upstreamModel)
	if err != nil {
		return nil, err
	}
...
func rewriteOpenAIVideoCreateModel(body []byte, upstreamModel string) ([]byte, error) {
	var value any
	...
	if err != nil {
		return nil, fmt.Errorf("rewrite video create body: %w", err)
	}
