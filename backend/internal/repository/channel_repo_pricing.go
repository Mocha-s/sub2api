...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID int64) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price, video_price_per_second, video_default_seconds, video_allowed_seconds, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	videoAllowedSecondsJSON, err := json.Marshal(pricing.VideoAllowedSeconds)
	if err != nil {
		return fmt.Errorf("marshal video allowed seconds: %w", err)
	}
	...
	result, err := r.db.ExecContext(ctx,
		...
		pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.VideoPricePerSecond, pricing.VideoDefaultSeconds,
		videoAllowedSecondsJSON, pricing.Platform, pricing.ID,
	)
	if err != nil {
...
func (r *channelRepository) batchLoadIntervals(ctx context.Context, pricingIDs []int64) (map[int64][]service.PricingInterval, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&iv.PerRequestPrice, &iv.VideoPricePerSecond, &iv.SortOrder, &iv.CreatedAt, &iv.UpdatedAt,
		); err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		var modelsJSON []byte
		var videoAllowedSecondsJSON []byte
		if err := rows.Scan(
			&p.ID, &p.ChannelID, &p.Platform, &modelsJSON, &p.BillingMode,
			&p.InputPrice, &p.OutputPrice, &p.CacheWritePrice, &p.CacheReadPrice,
			&p.ImageOutputPrice, &p.PerRequestPrice, &p.VideoPricePerSecond, &p.VideoDefaultSeconds,
		...
		p.VideoAllowedSeconds = unmarshalVideoAllowedSeconds(videoAllowedSecondsJSON)
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price, video_price_per_second, video_default_seconds, video_allowed_seconds)
...
func createIntervalExec(ctx context.Context, exec dbExec, iv *service.PricingInterval) error {
	return exec.QueryRowContext(ctx,
		...
		iv.InputPrice, iv.OutputPrice, iv.CacheWritePrice, iv.CacheReadPrice,
		iv.PerRequestPrice, iv.VideoPricePerSecond, iv.SortOrder,
	).Scan(&iv.ID, &iv.CreatedAt, &iv.UpdatedAt)
}

func unmarshalVideoAllowedSeconds(data []byte) []int {
	if len(data) == 0 {
		return nil
	}
	var seconds []int
