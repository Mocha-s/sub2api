...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	assertGroupMutationEnqueues := func(query, message string, args ...any) {
		_, err = integrationDB.ExecContext(ctx, query, args...)
		require.NoError(t, err)
		require.Equal(t, 1, count(), message)
		clear()
	}
	assertGroupMutationEnqueues("UPDATE groups SET allow_image_generation = NOT allow_image_generation WHERE id = $1", "image-generation permission changes must enqueue bound keys", group.ID)
	assertGroupMutationEnqueues("UPDATE groups SET allow_video_generation = NOT allow_video_generation WHERE id = $1", "video-generation permission changes must enqueue bound keys", group.ID)
	assertGroupMutationEnqueues("UPDATE groups SET max_reasoning_effort = 'high' WHERE id = $1", "reasoning effort ceiling changes must enqueue bound keys", group.ID)
	assertGroupMutationEnqueues("UPDATE groups SET reasoning_effort_mappings = '[{\"from\":\"high\",\"to\":\"medium\"}]'::jsonb WHERE id = $1", "reasoning effort mapping changes must enqueue bound keys", group.ID)
